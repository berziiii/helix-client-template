...
const addStyleComponent = (name) => {
    ...
    let string = `@import '../../app/${name.capitalize()}/${name}.styles';`
    fs.readFile(fileName, 'utf8', function (err, data) {
        ...
        fs.writeFile(fileName, result.join('\n'), 'utf8', function (err) {
            ...
            console.log(`${name.toUpperCase()}.STYLES.SCSS SUCCESSFULLY ADDED TO STYLES MANIFEST....`.cyan);
        });
    });
    return name;
};
...
const createStylesFile = (req) => {
    ...
    .then((name) => {
        console.log(`${name.toUpperCase()}.STYLES.SCSS SUCCESSFULLY CREATED....`.cyan);
        return name;
    })
