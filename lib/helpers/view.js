...
const writeViewLogic = function (name) {
    return new Promise((resolve, reject) => {
        ...
        let string = `const ${name.capitalize()} = require('./${name.capitalize()}/${name}.events.js');`;
        let present = false;
        fs.readFile(fileName, 'utf8', function (err, data) {
            if (err) {
                console.log(`${name.toUpperCase()} COMPONENT ROUTE FAILED!!`.red);
                reject(err);
            }
            ...
            fs.writeFile(fileName, result.join('\n'), 'utf8', function (err) {
                ...
                console.log(`${name.toUpperCase()} COMPONENT VIEW LOGIC ADDED SUCCESSFULLY....`.cyan);
            });
            resolve(name);
        });
    });
